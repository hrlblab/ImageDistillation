...
from utils_med import get_dataset, get_network, get_eval_pool, evaluate_synset, get_time, DiffAugment, ParamDiffAug
import wandb
import copy
import random
...
from confusemat import plot_confusion_matrix
        ...
        if it in eval_it_pool:
            for model_eval in model_eval_pool:
                ...
                for it_eval in range(args.num_eval):
                    ...
                    # if args.dataset == "ResNetMed":
                    #     # teacher_net.load_state_dict(torch.load("/Data/lmy_workspace/data_distillation/mtt-distillation/model/weights_pathmnist/resnet18_28_2.pth", map_location=args.device)['net'], strict=True)
                    #     pretrained_dict = \
                    #         torch.load(
                    #             "/Data/lmy_workspace/data_distillation/mtt-distillation/model/weights_pathmnist/resnet18_28_1.pth",
                    #             map_location=args.device)['net']
                    #
                    #     # Filter out unnecessary keys
                    #     model_dict = net_eval.state_dict()
                    ...
                    #     net_eval.load_state_dict(model_dict, strict=False)
                    eval_labs = label_syn
                    with torch.no_grad():
                ...
                true_labels = []
                pred_labels = []
                # Assuming you have the testloader ready and the model is already evaluated
                for inputs, labels in testloader:
                    inputs, labels = inputs.to(args.device), labels.to(args.device)
                    with torch.no_grad():
                        outputs = net_eval(inputs)  # net_eval should be your model in eval mode
                ...
                class_names = [str(i) for i in range(num_classes)]  # Adjust based on your class names
                f1 = f1_score(true_labels, pred_labels,
                              average='weighted')  # You can change the averaging method as required
                wandb.log({'F1_score/{}'.format(model_eval): f1}, step=it)  # Log the F1 score to wandb
        ...
        if it in eval_it_pool and (save_this_it or it % 1000 == 0):
            with torch.no_grad():
                ...
                if args.ipc < 50 or args.force_save:
                    ...
                    fig = plot_confusion_matrix(cm_percentage, class_names)
                    fig.savefig(os.path.join(save_dir, 'confusion_matrix_{}.png'.format(
                        it)))  # Save to the iteration-specific directory

                    # Log the confusion matrix figure to wandb
                    wandb.log({"Confusion Matrix": wandb.Image(fig)}, step=it)
