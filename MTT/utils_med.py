...
from networks import MLP, ConvNet, LeNet, AlexNet, VGG11BN, VGG11, ResNet18, ResNet18BN_AP, ResNet18_AP, ResNet_Med
from medmnist import PathMNIST, DermaMNIST, OCTMNIST, PneumoniaMNIST, RetinaMNIST, BloodMNIST, TissueMNIST, OrganAMNIST, OrganCMNIST, OrganSMNIST
...
# Compute the mean and std for my own datasets
def compute_mean_std(data_path):
    # Transform to convert images to tensors
    transform = transforms.Compose([transforms.ToTensor()])
    # Load the dataset
    dataset = datasets.ImageFolder(data_path, transform=transform)
    # Create a DataLoader without shuffling to speed up the loading
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=2)
    # Lists to hold channel-wise values
    channels_sum, channels_squared_sum, num_batches = 0, 0, 0

    for data, _ in dataloader:
        channels_sum += torch.mean(data ** 2, dim=[0, 2, 3])
...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
    ...
    elif dataset == 'dataset_800':
        channel = 3
        im_size = (32, 32)
        num_classes = 6
        mean = [0.7077, 0.5550, 0.6869]
        std = [0.1350, 0.1530, 0.0960]
        if args.zca:
            transform = transforms.Compose([transforms.ToTensor(),
                                            transforms.Resize(im_size),
                                            transforms.CenterCrop(im_size)])
        ...
        dst_train = datasets.ImageFolder(os.path.join(
            "/run/user/1443748932/gvfs/smb-share:server=hrlblab-nas2.it.vanderbilt.edu,share=data/lim47/Data Distillation/",
            "800_ds32_train"), transform=transform)
        dst_test = datasets.ImageFolder(os.path.join(
            "/run/user/1443748932/gvfs/smb-share:server=hrlblab-nas2.it.vanderbilt.edu,share=data/lim47/Data Distillation/",
            "800_ds32_test"), transform=transform)
        class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}

    elif dataset == 'dataset_800_ds64':
        channel = 3
        im_size = (64, 64)
        num_classes = 6
        mean = [0.7077, 0.5550, 0.6869]
        ...
        dst_train = datasets.ImageFolder(os.path.join(
            "/run/user/1443748932/gvfs/smb-share:server=hrlblab-nas2.it.vanderbilt.edu,share=data/lim47/Data Distillation/",
            "800_ds64_train"), transform=transform)
        dst_test = datasets.ImageFolder(os.path.join(
            "/run/user/1443748932/gvfs/smb-share:server=hrlblab-nas2.it.vanderbilt.edu,share=data/lim47/Data Distillation/",
            "800_ds64_test"), transform=transform)
        class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}
        ...
        dst_train = DataClass(split = 'train', transform=transform, download=True)
        dst_test = DataClass(split = 'test', transform=transform, download=True)
        class_names = info['label']
        class_map = {x: x for x in range(num_classes)}

    elif dataset == "PathMNIST_64":
        data_flag = 'pathmnist'
        ...
        DataClass = getattr(medmnist, info['python_class'])
        dst_train = DataClass(split = 'train', transform=transform, size=64, download=True)
        dst_test = DataClass(split = 'test', transform=transform, size=64, download=True)
        class_names = info['label']
        class_map = {x: x for x in range(num_classes)}
    ...
    elif dataset == "MedMNISTCombined":
        channel = 3
        im_size = (32, 32)
        num_classes = 8
        data_path = "/Data2/MedMNIST/MedMNIST_28/Sim_small"
        mean = [0.5, 0.5, 0.5]
        std = [0.5, 0.5, 0.5]
        if args.zca:
            transform = transforms.Compose([transforms.ToTensor(),
        ...
        dst_train = datasets.ImageFolder(os.path.join(
            "/Data2/MedMNIST/MedMNIST_28/Sim_small","train"), transform=transform)
        dst_test = datasets.ImageFolder(os.path.join(
            "/Data2/MedMNIST/MedMNIST_28/Sim_small","test"), transform=transform)
        class_names = dst_train.classes
        class_map = {x: x for x in range(num_classes)}
    ...
    testloader = torch.utils.data.DataLoader(dst_test, batch_size=128, shuffle=False,
                                             num_workers=0)  # Originally batch_size is 128
...
def get_network(model, channel, num_classes, im_size=(32, 32), dist=True):
    ...
    elif model == 'ResNetMed':
        net = ResNet_Med(channel=channel, num_classes=num_classes)
...
#         n_b = lab.shape[0]
#         output = net(img)
#         loss = criterion(output, lab.squeeze().long())
#
#         acc = np.sum(np.equal(np.argmax(output.cpu().data.numpy(), axis=-1), lab.cpu().data.numpy()))
#
...
#             all_preds.extend(torch.softmax(output, dim=1).cpu().numpy())
#
#         loss_avg += loss.item() * n_b
#         acc_avg += acc
#         num_exp += n_b
...
#     acc_avg /= num_exp
#
#     # Calculate AUC for each class if in evaluation mode
#     aucs = []
#     if mode != 'train':
#         all_labels_binarized = label_binarize(all_labels, classes=np.unique(all_labels))
#         for i in range(all_labels_binarized.shape[1]):  # Iterate over each class
#             # Compute AUC for the current class
#             if len(np.unique(all_labels_binarized[:, i])) == 2:  # Check if there are both positive and negative samples
#                 auc = roc_auc_score(all_labels_binarized[:, i], np.array(all_preds)[:, i])
#                 aucs.append(auc)
#             else:
#                 aucs.append(float('nan'))  # AUC is not defined if there are no positive or no negative samples
#
...
class MedMNISTSingleLabel(Dataset):
    def __init__(self, combined_folder, transform=None, dataset_type='train'):
        self.transform = transform

        # Load the combined datasets
        if dataset_type == 'train':
            self.images = np.load(os.path.join(combined_folder, 'combined_train_images.npy'))
            self.labels = np.load(os.path.join(combined_folder, 'combined_train_labels.npy'))
        elif dataset_type == 'test':
            self.images = np.load(os.path.join(combined_folder, 'combined_test_images.npy'))
            self.labels = np.load(os.path.join(combined_folder, 'combined_test_labels.npy'))
        else:
            raise ValueError("dataset_type must be either 'train' or 'test'")
        print(f"Labels shape: {self.labels.shape}")
        self.images = self.standardize_images(self.images)
        self.label_names = np.unique(self.labels)
    def standardize_images(self, images):
        # Standardize the images as required
        if images.ndim == 3:
